...
def get_filename(data_path=Path("./data"), filename="indosum", wiki_text=False, lang="id"):
...
def download_wiki_files(path, wiki_text=False, lang='id'):
        ...
        if not (path / "data_save.pkl").exists():
            gdown.download(
                lang_dict[lang]["data"], f"{path}/data_save.pkl", quiet=False
            )
        if wiki_text:
            gdown.download(
                lang_dict[lang["wiki"]],
                f"{path}/{lang}_wiki.txt",
                quiet=False,
            )
def main():
